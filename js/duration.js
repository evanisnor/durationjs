	...
	Duration.prototype.Parser.Standard = function(seconds, match) {
		...
		for (var groupIndex = 1; groupIndex < match.length; groupIndex++) {
			...
			if (/T/.test(value)) {
				...
				seconds += parseFloat(value.replace('Y', ''), 10) * cal.Seconds.per.Year;
			}
			else if (/\d+M/.test(value) && !hasFoundT) {
				seconds += parseFloat(value.replace('M', ''), 10) * cal.Seconds.per.Month;
			}
			else if (/\d+D/.test(value)) {
				seconds += parseFloat(value.replace('D', ''), 10) * cal.Seconds.per.Day;
			}
			else if (/\d+H/.test(value)) {
				seconds += parseFloat(value.replace('H', ''), 10) * cal.Seconds.per.Hour;
			}
			else if (/\d+M/.test(value) && hasFoundT) {
				seconds += parseFloat(value.replace('M', ''), 10) * cal.Seconds.per.Minute;
			}
			else if (/\d+S/.test(value)) {
				seconds += parseFloat(value.replace('S', ''), 10);
			}
			else if (/\d+[A-Z]/.test(value)) {
				throw new Error(Duration.prototype.Error.UnexpectedFormat);
			}
